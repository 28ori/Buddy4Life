...
var url = `https://${process.env.DOG_API_HOST}`
...
const options = {
    ...
    const get = async (req: Request, res: Response) => {
        try {
            ...
            res.status(500).json({ message: "Error retrieving all dogs types with error: " + err.message });
    ...
    const getById = async (req: Request, res: Response) => {
        try {
            ...
            res.status(500).json({ message: `Error retrieving dog type: ${req.params.id} with error: ` + err.message });
